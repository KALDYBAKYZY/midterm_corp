          ...
          <div key={task.id} className="list-group-item d-flex justify-content-between align-items-center">
            <div>
              <div className="form-check">
                <input className="form-check-input" type="checkbox" checked={task.completed} onChange={()=>toggleCompleted(task)} id={`chk-${task.id}`} />
                <label className="form-check-label ms-2" htmlFor={`chk-${task.id}`}>
                  <strong>{task.title}</strong> <small className="text-muted">{task.user?.name ? ` — ${task.user.name}` : ''}</small>
                </label>
              </div>
